<template>
  ...
  >
    ...
    >
      ...
      <section>
        <h2 :style="{ color: accent, fontSize: '1em', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '1em' }">
          Contact
        </h2>
        ...
        <section v-if="hasContent(resume, key)">
          ...
          <template v-if="key === 'skills'">
            <div style="display: flex; flex-wrap: wrap; gap: 0.5em;">
              <span
                ...
                :key="s"
                :style="{ background: `${accent}30`, color: 'white', border: `1px solid ${accent}`, padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em' }"
              >
                {{ s }}
              </span>
            </div>
          ...
          <template v-else-if="key === 'languages'">
            ...
            >
              ...
              <div :style="{ height: '4px', width: '100%', background: '#334155', marginTop: '4px', borderRadius: '2px' }">
                <div :style="{ height: '100%', width: getLevelWidth(l.level), background: accent, borderRadius: '2px' }" />
              </div>
            </div>
          </template>
    ...
    <main style="padding: 0.6in 0.5in; display: flex; flex-direction: column; gap: 2em;">
      <header>
        <h1 :style="{ margin: 0, fontSize: '3.2em', fontWeight: 900, color: '#0f172a', lineHeight: 1, letterSpacing: '-0.03em' }">
      ...
      >
        <section v-if="hasContent(resume, key)">
          <h2 :style="{ fontSize: '1.4em', fontWeight: 800, color: '#0f172a', marginBottom: '0.8em', display: 'flex', alignItems: 'center', gap: '0.5em' }">
            <span :style="{ width: '12px', height: '12px', background: accent, borderRadius: '2px' }" />
            {{ getSectionTitle(key) }}
          </h2>
          ...
          <template v-else-if="key === 'experience'">
            ...
            >
              <div :style="{ position: 'absolute', left: '-6px', top: '8px', width: '10px', height: '10px', borderRadius: '50%', background: accent }" />
              <div style="display: flex; justify-content: space-between; align-items: baseline;">
                <div style="font-weight: 800; font-size: 1.1em;">
                  {{ e.role }}
                </div>
                <div :style="{ color: accent, fontWeight: 700, fontSize: '0.85em' }">
          ...
          <template v-else-if="key === 'projects'">
            ...
            >
              ...
              <div
                v-if="p.link"
                :style="{ color: accent, fontSize: '0.85em', margin: '2px 0 6px' }"
              >
                {{ p.link }}
              </div>
